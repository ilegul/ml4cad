...
artifacts was destroyed during the refactor, together with the thesis documents.
Those documents were restored from the checkpoint commit, verified
byte-identical, and later moved out of the repository entirely. The superseded artifacts were only partly recoverable: the 82
...
The four thesis documents `reports/Report_*.docx` are not part of the
deliverable. They were not generated by this pipeline, and they were removed
from the repository and from its history, tags included, so that nothing
published contains them. The files themselves are preserved unchanged outside
the repository. Raw workbooks in `data/raw` were never read for writing.
...
One addition sits outside the compact reference layout, deliberately:
...
models/  predictions/  results/  figures/  cache/
